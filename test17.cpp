...
void Test17()
{
	cout << "Test17: Attempting disjoint graph solution via GPU.\n";
...
unsigned int BitCount(struct bitset_t* b)
{
...
bool IsSet(__global struct bitset_t* b, unsigned int index)
{
...
void AndGlobal(struct bitset_t* dest, __global struct bitset_t* lhs, __global struct bitset_t* rhs)
{
...
void AndPrivate(struct bitset_t* dest, struct bitset_t* lhs, __global struct bitset_t* rhs)
{
	dest->data0 = lhs->data0 & rhs->data0;
	dest->data1 = lhs->data1 & rhs->data1;
	dest->data2 = lhs->data2 & rhs->data2;
...
void Clear(struct bitset_t* b, unsigned int index)
{
...
__kernel void test(__global struct bitset_t* m, __global struct counts_t* counts, unsigned long size)
{
	...
	unsigned long i1 = remainder % size;
	remainder /= size;
	unsigned long i2 = remainder % size;
	...
	unsigned int c3 = BitCount(&r2);
	if (c3 == 0)
		return;
	count->count3 += c3;

	for (unsigned long i3 = i2; c3--; )
	{
		i3 = CountLeadingZeros(&r2, i3 + 1);
		Clear(&r2, i3);
		struct bitset_t r3;
		AndPrivate(&r3, &r2, m + i3);
		unsigned int c4 = BitCount(&r3);
		if (c4 == 0)
			continue;
		count->count4 += c4;
